...
def readExpData(fn, mode='raw'):
    ...
    exonpos = sp.array(pandas.read_csv(fn, delim_whitespace=True, usecols=[0], skiprows=2,
                                       engine='c'))  # sp.loadtxt(fn, delimiter = '\t', dtype = 'string', usecols = [0], skiprows = 2))
    data = sp.array(pandas.read_csv(fn, delim_whitespace=True, dtype='float', usecols=myCols, skiprows=2,
                                    engine='c'))  # sp.loadtxt(fn, delimiter = '\t', dtype = 'float',  usecols = myCols, skiprows = 2))
...
def readExpDataBam(base_dir):
    ...
    for i, f in enumerate(allfiles):
        if i == 0:
            ...
            data = sp.array(pandas.read_csv(os.path.join(base_dir, f),
                                            delim_whitespace=True))  # sp.loadtxt(os.path.join(base_dir, f), delimiter = '\t', dtype = 'string')
            ...
            tmp = sp.array(pandas.read_csv(os.path.join(base_dir, f),
                                           delim_whitespace=True))  # sp.loadtxt(os.path.join(base_dir, f), delimiter = '\t', dtype = 'string')
